...
Model LPSolver::linearProgram(const Model mo) {
	...
	SCIPcreateProb(scip, "planning", NULL, NULL, NULL, NULL, NULL, NULL, NULL); //sens par d�faut : Min

	// Liste contraintes
	vector<SCIP_CONS*> consVec = vector<SCIP_CONS*>();
	// Liste variables
	vector<VariableData*> varsData = vector<VariableData*>(); //contient les variables SCIP + donn�es li�es (id, agent, jour, poste)
	...
	vector<Agent*> agents_service_i;
	vector<Agent*> agents;

	for (auto s : mr.getServices()) {
		agents_service_i = mr.getAgentFrom(s);
		agents.insert(agents.end(), agents_service_i.begin(), agents_service_i.end());
	}
	...
	for (int day = 0; day < mr.getNbDays(); day++) {
		...
		for (auto s : mr.getServices()) {
			cout << "Service " << s->getId() << endl;
			//d�faut : ne prend pas en compte les agents qu'on peut solliciter en dehors du service o� ce poste est d�di� en g�n�ral
			agents_service_i = mr.getAgentFrom(s);

			for (auto job : s->getPosts()) { //pour g�n�raliser : mr.getPosts() mais il faudrait d'autres changements
				if (job->getId() != "Repos") {
	...
	vector<VariableData*> varsDataTmp(varsData); //sert � ne pas re-avoir les agents d�j� vus
	int nb_vals = 0;
	for (auto a : agents) {
		nb_vals = 0;
		long long coeffs_hours[500];  //taille de tableau en dur � changer d�s que possible
		...
		for (int i = 0; i < varsDataTmp.size(); i++) {
			if (a->getId() == varsDataTmp[i]->getAgent()->getId()){ //&& varsDataTmp[i]->getVariable() >= 0) {
				...
				//varsDataTmp.erase(varsDataTmp.begin()+i); //� l'air de faire buguer la contrainte
				nb_vals++;
			}
		}
		...
		//modifier la contrainte pour la cr�er plut�t avec une contrainte lin�aire :
	...
	varsDataTmp = varsData; //pour ne pas re-avoir les agents d�j� vus
	nb_vals = 0;
	int jour_semaine = 1; 
	for (auto a : agents) {
		jour_semaine = 1;
		nb_vals = 0;
		long long coeffs_hours[500];
		SCIP_VAR* varsConsHours[500]; //maximum 7*nbPosts vars par semaine
		for (int day = 0; day < mr.getNbDays(); day++) {
	...
	for (auto a : agents) {
		...
		int nb_vals_ji[200]; //nb_vals pour le jour i de l'agent courant
		...
		for (int day = 0; day < mr.getNbDays(); day++) {
			...
			if (jour_semaine >= 7) {
				...
				if (jour_semaine >= 8) {
					...
					for (int i = 0; i < nb_vals_ji[i_boucle_jour]; i++) {
	...
	*/
		...
		for (auto s : mr.getServices()) {
			if (day < mr.getNbDays() - 1) {
				for (auto a : agents) {
					...
					// V�rifier si l'agent ne travaille pas d�j� de jour le jour suivant
			...
			// 3 jours d'affil�e possible en GHR, SDC et UK
	...
	//stocke les donn�es du PL dans un fichier de log
	...
	for (auto varData : varsData) {
		...
		else if(varValue == .0 && varData->getAgent()->getCalendar()[varData->getDay()] == NULL){ //si on a pas un autre poste attribu� plus t�t pour le m�me jour et le m�me agent ==> repos
